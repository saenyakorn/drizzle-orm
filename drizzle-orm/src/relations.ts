...
export abstract class Relation<
  ...
  TForeignTable extends AnyTable<{ name: TTableName }> = AnyTable<{
    name: TTableName;
  }>,
> {
  ...
  constructor(
    readonly sourceTable: TSourceTable,
    readonly referencedTable: TForeignTable,
    readonly relationName: string | undefined,
  ) {
    this.referencedTableName = referencedTable[Table.Symbol.Name] as TTableName;
  }

  abstract withFieldName(
    fieldName: string,
  ): Relation<TTableName, TSourceTable, TForeignTable>;
}
...
> extends Relation<TTableName, TSourceTable, TForeignTable> {
  ...
  withFieldName(
    fieldName: string,
  ): One<TTableName, TIsNullable, TSourceTable, TForeignTable> {
...
> extends Relation<TTableName, TSourceTable, TForeignTable> {
  ...
  withFieldName(
    fieldName: string,
  ): Many<TTableName, TSourceTable, TForeignTable> {
    const relation = new Many<TTableName, TSourceTable, TForeignTable>(
      this.sourceTable,
      this.referencedTable,
      this.config,
    );
...
export function relations<
  ...
  TSourceTable extends AnyTable<{ name: TTableName }>,
>(
  table: TSourceTable,
  relations: (
    helpers: TableRelationsHelpers<TTableName, TSourceTable>,
  ) => TRelations,
  ...
  return new Relations<TTableName, TRelations>(
    ...
    (helpers) =>
      Object.fromEntries(
        Object.entries(
          relations(helpers as TableRelationsHelpers<TTableName, TSourceTable>),
...
export function createMany<TSourceTable extends Table>(
  sourceTable: TSourceTable,
) {
  return function many<
    TTableName extends string,
    TForeignTable extends AnyTable<{ name: TTableName }>,
  >(
    referencedTable: TForeignTable,
    config?: { relationName: string },
  ): Many<TForeignTable['_']['name'], TSourceTable, TForeignTable> {
...
>(sourceTable: TSourceTable) {
  return {
    one: createOne<TTableName, TSourceTable>(sourceTable),
    many: createMany<TSourceTable>(sourceTable),
  };
}

export type TableRelationsHelpers<
  TTableName extends string,
  TSourceTable extends Table = Table,
> = ReturnType<typeof createTableRelationsHelpers<TTableName, TSourceTable>>;
